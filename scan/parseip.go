...
func ParseIP(cidrs []string) (hosts []string, err error) {
	for _, cidr := range cidrs {
		hosts = append(hosts, ParseIPs(cidr)...)
	}
...
func ParseIPs(ip string) (hosts []string) {
	...
	switch {
	case ip == "192":
		return ParseIPs("192.168.0.0/8")
	case ip == "172":
		return ParseIPs("172.16.0.0/12")
	case ip == "10":
		return ParseIPs("10.0.0.0/8")
